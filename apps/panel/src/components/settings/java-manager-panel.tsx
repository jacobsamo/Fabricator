      ...
      <CardContent className="grid gap-4">
        ...
        {taskId ? (
          <div className="grid gap-2">
            ...
            <div className="h-2 overflow-hidden rounded bg-border"><div className="h-full bg-primary" style={{ width: `${pct}%` }} /></div>
          ...
          <div className="flex flex-wrap items-end gap-2 border-t border-border pt-4">
            <label className="grid gap-1 text-xs font-semibold uppercase text-muted-foreground">Add a Java runtime
              <select className="h-9 min-w-40 rounded-md border border-input bg-background px-3 text-sm" value={selectedMajor} onChange={(event) => setSelectedMajor(Number(event.target.value))} disabled={availableMajors.length === 0}>
                {availableMajors.length === 0 ? <option>All common versions installed</option> : availableMajors.map((major) => <option key={major} value={major}>Java {major}</option>)}
              </select>
            </label>
            <Button size="sm" disabled={availableMajors.length === 0} onClick={async () => { const task = await installJava(selectedMajor); setTaskId(String(task.task_id)); }}>Install</Button>
          </div>
        )}
        {deleteMajor !== null ? (
          <div className="rounded-md border border-destructive/50 bg-destructive/10 p-3">
            <p className="text-sm">Remove Java {deleteMajor}? Servers that need it will prompt for reinstall next start.</p>
            <div className="mt-3 flex justify-end gap-2"><Button variant="ghost" size="sm" onClick={() => setDeleteMajor(null)}>Cancel</Button><Button variant="destructive" size="sm" onClick={async () => { await uninstallJava(deleteMajor); setDeleteMajor(null); await installed.refetch(); }}>Remove</Button></div>
          </div>
        ) : null}
      </CardContent>
    </Card>
